...
| **SciQ** | Short-form | 0.940 | **0.954** | 0.933 | 0.902 | 0.934 | 0.890 |
...
|---|---|---|---|
| SciQ | 0.954 (MiniLM) | 0.934 (MiniLM) | **↓ 下降** |
...
- 对于 **TruthfulQA**：CoT 推理抑制了模型常见的"自信错误"（如 "veins appear blue because blood is blue"），thinking 模式下 Accuracy 虽略降（35.0% → 28.2%），但正确答案与错误答案之间的 cosine 差距更清晰（bge 均值差异：0.113 → 0.130，+0.017），AUC 反而提升。

#### 2.3 嵌入模型对比

- **E5-base-v2** 最稳健：在 4 个数据集 × 2 模式共 8 个组合中，5 次位列最优或次优，尤其在长文本任务上表现突出（NQ: 0.934，TruthfulQA: 0.813）。
- **MiniLM-L6-v2** 在 SciQ 上最强（0.954），但长文本任务上最弱（TruthfulQA: 0.723），极化明显。
...
|---|---|---|
| SciQ | 52.2% (261/500) | 60.8% (304/500) |
...
|---|---|---|---|
| SciQ (no_thinking) | 29 | 40 | 词汇变体 (14)、语义歧义 (16)、其他 (39) |
...
- **FN 相似度均值低于阈值说明真正失败**：SciQ no_thinking 的 FN 均值 0.788 仍高于阈值，显示这些案例确为模型难以区分的边界样本，而非明显错误。
...
| 跨任务最鲁棒 | E5-base-v2 + no_thinking | 平均 ~0.895 |
...
1. TruthfulQA 的 FP 均值高达 **0.865**（bge no_thinking），这意味着错误答案与正确答案在欧式嵌入空间几乎无法分离；层次化几何（双曲空间）能更好捕捉答案之间的语义层次关系（如"Fortune cookies originated in China" 与 "... in the U.S." 的矛盾关系）。
...
### 生成的文件
- results/improvement_summary.csv（121行完整对比表）
...
|---|---|---|
| BGE-base | 0.9340 | 0.0690 |
| MiniLM-L6 | 0.9368 | 0.0526 |
| E5-base | **0.9517** | 0.1225 |

**测试集 ΔAUC vs baseline（仅 test 子集对比）：**

| 数据集 | 模式 | ΔAUC (BGE) | ΔAUC (MiniLM) | ΔAUC (E5) |
|---|---|---|---|---|
| SciQ | no_thinking | **+0.030** | **+0.011** | **+0.035** |
| SciQ | thinking | **+0.026** | **+0.002** | **+0.025** |
| SimpleQA | no_thinking | **+0.035** | **+0.027** | **+0.103** |
| SimpleQA | thinking | ≈0 | ≈0 | ≈0 |
| NQ | no_thinking | -0.009 | **+0.055** | -0.073 |
| NQ | thinking | +0.009 | **-0.189** | -0.042 |
| TruthfulQA | no_thinking | **+0.011** | **+0.072** | +0.003 |
| TruthfulQA | thinking | **+0.123** | **+0.105** | **+0.071** |

**关键规律：HELM 对 TruthfulQA/thinking 有一致正效果（BGE +12.3%, MiniLM +10.5%, E5 +7.1%），对 SciQ 和 SimpleQA/no_thinking 有轻微至中等正效果（+1%~+10%）；SimpleQA/thinking 基线测试子集已饱和（AUC≈1.0），增益为零；NQ/thinking 结果不稳定（BGE +0.9%, MiniLM -18.9%），因 test 子集 n_pos 仅 4 条，方差较大**

**根本原因分析：**
- **有效场景（TruthfulQA/thinking）**：CoT 推理产生的推断结构（层次化语义）在 Poincaré Ball 中被更好捕捉。双曲空间的树形度量天然适合"证据链"结构，正确与错误答案在曲率加持下可分性增强；TruthfulQA 的反事实结构（需区分"常识谬误"与"正确答案"）正适合双曲几何建模
- **SciQ/SimpleQA no_thinking 正效果**：重新训练的 HELM 模型 val AUC 大幅提升（0.934/0.937/0.952 vs 旧版 0.897/0.900/0.919），投影头质量改善带来整体提升
- **NQ/thinking 不稳定**：test 子集 n_pos=4，n_neg=53，样本量极小，AUC 估计方差高；MiniLM 的 -18.9% 为统计异常，不具代表性
...
| 数据集 | 模式 | ΔAUC (BGE) | ΔAUC (MiniLM) | ΔAUC (E5) |
|---|---|---|---|---|
| SciQ | no_thinking | ≈0.000 | ≈0.000 | -0.003 |
...
| **HELM (trained)** | **TruthfulQA thinking: +12.3%** | NQ thinking MiniLM: -18.9%（n_pos=4，高方差） | **TruthfulQA 一致提升，NQ 不稳定** |
