...
5. [Results](#5-results)
   ...
   - [5.5 TA Baseline Compatibility Experiments](#55-ta-baseline-compatibility-experiments)
...
- A compatibility check against the TA baseline pipeline and datasets, including full-size SciQ/SimpleQuestions predictions and HHEM/MPNet artifacts from the original TA scripts
...
### 5.5 TA Baseline Compatibility Experiments

To validate that the evaluation code can consume the TA baseline data layout, we added `scripts/evaluate_ta_baseline_compatibility.py` and analyzed the existing outputs from `NLI Project v2/run_all_datasets.sh`. The reproducible summary is generated by `scripts/analyze_added_experiments.py` and written to `results/added_experiments/`.

**Compatibility predictions from the NLP pipeline.** The compatibility script has completed no-thinking generation for the two short-form TA datasets. On the full TA-prepared splits, the same exact/contains correctness rule gives:

| Dataset | Mode | n | Exact | Contains/Correct | Mean prediction words | Max words |
|---|---|---:|---:|---:|---:|---:|
| SciQ | no_thinking | 13,679 | 41.3% | **55.8%** | 1.89 | 196 |
| SimpleQuestionsWiki | no_thinking | 49,202 | 12.1% | **16.6%** | 2.83 | 222 |
